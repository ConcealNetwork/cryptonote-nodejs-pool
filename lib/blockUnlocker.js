...
const runInterval = async () => {
    ...
    if (isRunning) {
        // Don't schedule another call - the interval will fire again naturally
        return;
    }
    ...
    try {
        ...
        const blockChecks = await Promise.all(blocks.map(async (block) => {
            try {
                ...
                // Only log when block is actually unlocked (not during normal checking)
                if (block.unlocked) {
                    log('info', logSystem, 'Block %d unlocked: orphaned=%d, depth=%d, reward=%d', [block.height, block.orphaned, blockHeader.depth, block.reward]);
                }
        ...
        for (const block of unlockedBlocks) {
            ...
            let feePercent = config.blockUnlocker.poolFee / 100;
                ...
                for(const wallet in donations) {
                    const percent = donations[wallet] / 100;
                        ...
                        // Donation processing - only log if amount is significant
                        if (payments[wallet] > 0) {
                            log('info', logSystem, 'Block %d donation: %s = %d', [block.height, wallet, payments[wallet]]);
                        }
                    }
                }
            ...
            const reward = Math.round(block.reward - (block.reward * feePercent));
                ...
                // Only log summary when block is unlocked (not per-block details)
                ...
                for (const worker of Object.keys(block.workerScores)) {
                    ...
                    redisClient.zAdd(`${config.coin}:blocksMiner:matured:${worker}`, {
                        score: block.height,
                        value: [
                            block.hash,
                            block.time,
