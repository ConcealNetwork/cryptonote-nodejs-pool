...
const emailSystem = require('./email.js');
const telegram = require('./telegram.js');
const utils = require('./utils.js');

// Initialize log system
const logSystem = 'notifications';
...
function sendToTelegramChannel(id, variables) {
    ...
    if (config.telegram && config.telegram.enabled) {
        const message = getTelegramMessage(id, variables);
        if (!message || message === '') {
...
function sendToMinerTelegram(miner, id, variables) { 
    ...
    if (config.telegram && config.telegram.enabled) {
        ...
        redisClient.hGet(config.coin + ':telegram', miner, (error, chatId) => {
...
function sendBlockTelegram(id, variables) {
    ...
    if (config.telegram && config.telegram.enabled) {
        ...
        redisClient.hGetAll(`${config.coin}:telegram:blocks`, (error, data) => {
...
function sendToAllEmails(id, variables) {
    ...
    if (config.email && config.email.enabled) {
        const subject = getEmailSubject(id, variables);
        const content = getEmailContent(id, variables);
        if (!content || content === '') {
            log('info', logSystem, 'Notification disabled for %s: empty email content.', [id]);
            return;
        }

        redisClient.hGetAll(`${config.coin}:notifications`, (error, data) => {
            if (error || !data) return ;
...
function sendToMinerEmail(miner, id, variables) {
    ...
    if (config.email && config.email.enabled) {
        ...
        redisClient.hGet(`${config.coin}:notifications`, miner, (error, email) => {
...
function getEmailSubject(id, variables) {
    const subject = replaceVariables(notificationSettings.emailSubject[id], variables) || '';
    return subject;
}

// Get email content
function getEmailContent(id, variables) {
    const message = notificationSettings.emailMessage[id] || '';
...
function getTelegramMessage(id, variables) {
    ...
    const message = replaceVariables(notificationSettings.telegramMessage[id], telegramVars) || '';
    return message;
}
