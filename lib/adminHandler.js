...
class AdminHandler {
    ...
    async handleAdminStats(response) {
        ...
        try {
            ...
            const _hashrateCutoff = now - this.config.api.hashrateWindow;
            ...
            try {
                const walletRpcResult = await new Promise((resolve, _reject) => {
        ...
        try {
            ...
            for (const key of workerKeys) {
                ...
                workersData[address] = {
                    ...
                    minPayoutLevel: parseInt(data.minPayoutLevel, 10) || 0,
                };
            }
    ...
    async handleAdminMonitoring(response) {
        ...
        try {
            ...
            const [monitoring, logs] = await Promise.all([
                ...
                new Promise((resolve, reject) => this.getLogFiles((err, data) => (err ? reject(err) : resolve(data)))),
            ]);
                ...
                async (error, result) => {
                    ...
                    try {
                        ...
                        this.log(
                            'error',
                            this.logSystem,
                            'CRITICAL: Payment sent but Redis update failed! TxHash: %s, Error: %j',
                            [txHash, redisError]
                        );
                        response.end(
                            JSON.stringify({
    ...
    handleAdminSendFusion(urlParts, response) {
        ...
        // Validate anonymity: if undefined or out of range (0-5), use config default
        let anonymity = urlParts.query.anonymity !== undefined 
            ? parseInt(urlParts.query.anonymity, 10) 
            : undefined;
        
        if (anonymity === undefined || isNaN(anonymity) || anonymity < 0 || anonymity > 5) {
            anonymity = this.config.wallet?.anonymity || 5;
        }
        ...
        this.log('info', this.logSystem, 'Sending fusion transaction for threshold: %d, anonymity: %d, address: %s', [
            threshold,
            anonymity,
            this.config.poolServer.poolAddress,
        ]);
    ...
    /**
     * Administration: wallet operations (JSON-RPC proxy)
     **/
    handleAdminWalletOps(request, response) {
        response.writeHead(200, {
            'Access-Control-Allow-Origin': this.corsOrigin,
            'Access-Control-Allow-Credentials': 'true',
            'Content-Type': 'application/json',
        ...
        request.on('end', () => {
            try {
                ...
                this.log('info', this.logSystem, 'Wallet OPS request: method=%s, params=%j', [method, params]);

                // For getTransactions, add pool address if addresses not provided
                if (method === 'getTransactions' && !params.addresses) {
                    params.addresses = [this.config.poolServer.poolAddress];
                }

                // Forward to wallet RPC
                this.apiInterfaces.rpcWallet(
                    method,
                    params,
                    (error, result) => {
                        if (error) {
                            this.log('error', this.logSystem, 'Wallet OPS error: %j', [error]);
                            response.end(
                                JSON.stringify({
                                    jsonrpc: '2.0',
                                    id: id,
                        ...
                        this.log('info', this.logSystem, 'Wallet OPS result: %j', [result]);
                        response.end(
                            JSON.stringify({
                                jsonrpc: '2.0',
                                id: id,
                ...
                this.log('error', this.logSystem, 'Error parsing wallet OPS request: %j', [error]);
                response.end(
                    JSON.stringify({
                        jsonrpc: '2.0',
                        id: null,
        ...
        request.on('error', (error) => {
            this.log('error', this.logSystem, 'Request error in wallet OPS: %j', [error]);
            response.end(
                JSON.stringify({
                    jsonrpc: '2.0',
                    id: null,
