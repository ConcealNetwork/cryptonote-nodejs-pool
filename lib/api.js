    ...
    try {
        ...
        switch (urlParts.pathname) {
            ...
            case '/admin_login':
                ...
                break;
            case '/admin_logout':
                authManager.handleAdminLogout(request, response);
                break;
            case '/admin_stats':
            ...
            case '/admin_manual_payment':
                ...
                break;
            case '/admin_wallet_balance':
                if (!authManager.authorize(request, response)) {
                    return;
                }
                adminHandler.handleAdminWalletBalance(response);
                break;
            case '/admin_estimate_fusion':
                if (!authManager.authorize(request, response)) {
                    return;
                }
                adminHandler.handleAdminEstimateFusion(urlParts, response);
                break;
            case '/admin_send_fusion':
                if (!authManager.authorize(request, response)) {
                    return;
                }
                adminHandler.handleAdminSendFusion(urlParts, response);
                break;
            case '/admin_wallet_ops':
                if (!authManager.authorize(request, response)) {
                    return;
                }
                adminHandler.handleAdminWalletOps(request, response);
                break;
...
function collectStats() {
    ...
    const getConfig = Promise.resolve({
        ...
        updateInterval: config.api.updateInterval,
        ...
        wallet: {
            anonymity: config.wallet?.anonymity || 5
        }
    });
    ...
    if (longpoll) {
        ...
        redisClient
            ...
            .then((result) => {
                ...
                // Use cryptographically secure random value instead of Math.random()
