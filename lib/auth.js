...
class AuthManager {
    constructor(config, log, logSystem) {
        ...
        // Use persistent JWT secret from config
        const isProduction = process.env.NODE_ENV === 'production';
        
        if (!config.api?.jwtSecret) {
            if (isProduction) {
                throw new Error('jwtSecret is required in production. Run: node scripts/generate-jwt-secret.js');
            }
            // Development: use random secret (tokens invalidated on restart)
        ...
        if (isProduction && !config.api?.ssl?.enabled) {
