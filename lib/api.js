...
function collectStats(){
    ...
    async.parallel({
        pool: (callback)=> {
            redisClient.multi(redisCommands).exec((error, replies)=> {
        ...
        charts: (callback) => {
            ...
            charts.getPoolChartsData((error, data) => {
                ...
                redisClient.zrevrange(config.coin + ':blocks:matured', 0, -1, 'WITHSCORES', (err, result) => {
...
function getNetworkData(callback, rpcMode) {
    ...
    if (rpcMode === 'get_info') {
        apiInterfaces.rpcDaemon('get_info', {}, (error, reply)=> {
            if (error) {
...
function handleMinerStats(urlParts, response){
    ...
    if (longpoll){
        ...
        redisClient.exists(config.coin + ':workers:' + address, (error, result)=> {
            if (!result){
...
function handleTopMiners(response) {
    async.waterfall([
        ...
        (workerKeys, callback) => {
            var redisCommands = workerKeys.map((k) => ['hmget', k, 'lastShare', 'hashes']);
            redisClient.multi(redisCommands).exec((error, redisData) => {
                var minersData = [];
...
function handleGetMinerPayoutLevel(urlParts, response){
    ...
    redisClient.hget(config.coin + ':workers:' + address, 'minPayoutLevel', (error, value)=> {
        if (error){
...
function handleSetMinerPayoutLevel(urlParts, response){
    ...
    minerSeenWithIPForAddress(address, ip, (error, found) => {
        ...
        redisClient.hset(config.coin + ':workers:' + address, 'minPayoutLevel', payoutLevel, (error, value)=> {
            if (error){
...
function handleGetMinerNotifications(urlParts, response){
    ...
    redisClient.hget(config.coin + ':notifications', address, (error, value)=> {
        if (error){
...
function handleSetMinerNotifications(urlParts, response){
    ...
    minerSeenWithIPForAddress(address, ip, (error, found) => {
        ...
        if (action === "enable") {
            ...
            redisClient.hset(config.coin + ':notifications', address, email, (error, value)=> {
                if (error){
            ...
            redisClient.hdel(config.coin + ':notifications', address, (error, value)=> {
                if (error){
...
function handleGetTelegramNotifications(urlParts, response){
    ...
    if (type == 'default') {
        redisClient.hget(config.coin + ':telegram:default', chatId, (error, value)=> {
            if (error){
    ...
    // Blocks notification
    if (type === 'blocks') {
        redisClient.hget(config.coin + ':telegram:blocks', chatId, (error, value)=> {
            if (error){
    ...
    if (type === 'miner') {
        ...
        redisClient.hget(config.coin + ':telegram', address, (error, value)=> {
            if (error){
                response.end(JSON.stringify({'status': 'Unable to get current telegram chat id from database'}));
                return;
            }
...
function handleSetTelegramNotifications(urlParts, response){
    ...
    if (type == 'default') {
        ...
        redisClient.hset(config.coin + ':telegram:default', chatId, address, (error, value)=> {
            if (error){
    ...
    if (type === 'blocks') {
        // Enable
        if (action === "enable") {
            redisClient.hset(config.coin + ':telegram:blocks', chatId, 1, (error, value)=> {
                if (error){
        ...
        // Disable
            ...
            redisClient.hdel(config.coin + ':telegram:blocks', chatId, (error, value)=> {
                if (error){
    ...
    if (type === 'miner') {
        ...
        redisClient.exists(config.coin + ':workers:' + address, (error, result)=> {
            ...
            // Enable
            if (action === "enable") {
                redisClient.hset(config.coin + ':telegram', address, chatId, (error, value)=> {
                    if (error){
                        response.end(JSON.stringify({status: 'Unable to enable telegram notifications'}));
                        return;
                    }
            ...
            // Disable
                ...
                redisClient.hdel(config.coin + ':telegram', address, (error, value)=> {
                    if (error){
                        response.end(JSON.stringify({status: 'Unable to disable telegram notifications'}));
                        return;
                    }
...
function handleAdminStats(response){
    async.waterfall([
        ...
        //Get worker balances
        (workerKeys, blocks, callback)=> {
            var redisCommands = workerKeys.map((k)=> ['hmget', k, 'balance', 'paid']);
            redisClient.multi(redisCommands).exec((error, replies)=> {
                if (error){
...
function handleAdminUsers(response){
    async.waterfall([
        ...
        // get workers data
        (workerKeys, callback) => {
            var redisCommands = workerKeys.map((k) => ['hmget', k, 'balance', 'paid', 'lastShare', 'hashes']);
            redisClient.multi(redisCommands).exec((error, redisData) => {
                var workersData = {};
                for(var i in redisData) {
                    var keyParts = workerKeys[i].split(':');
...
function handleAdminPorts(response){
    async.waterfall([
        ...
        (portsKeys, callback) => {
            var redisCommands = portsKeys.map((k) => ['hmget', k, 'port', 'users']);
            redisClient.multi(redisCommands).exec((error, redisData) => {
                var portsData = {};
                for (var i in redisData) {
...
function getMonitoringData(callback) {
    ...
    redisClient.multi(redisCommands).exec((error, results) => {
        var stats = {};
        for(var i in modules) {
            if(results[i]) {
...
function minerSeenWithIPForAddress(address, ip, callback) {
    ...
    redisClient.sismember([config.coin + ':workers_ip:' + address, ip], (error, result) => {
...
if (config.api.ssl){
    if (!config.api.sslCert) {
        ...
        var ssl_server = https.createServer(options, (request, response)=> {
            if (request.method.toUpperCase() === "OPTIONS"){
                response.writeHead("204", "No Content", {
                    "access-control-allow-origin": '*',
                    "access-control-allow-methods": "GET, POST, PUT, DELETE, OPTIONS",
