...
var log = (severity, system, text, data)=> {
...
if (cnVariant === 3) {
    ...
    if (!(cnHashing && cnHashing.cryptonight_gpu)) {
...
// Difficulty buffer
var diff1 = new BN('FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF', 16);
...
function jobRefresh(loop, callback){
    ...
    getBlockTemplate((error, result)=> {
        const buffer = new Buffer(result.blocktemplate_blob, 'hex');
        const new_hash = new Buffer(32);
        ...
        if (!poolStarted) {
            startPoolServerTcp((successful)=> { poolStarted = true });
        }
        callback(true);
    })
}
...
Miner.prototype = {
    ...
    getTargetHex: function(){
        ...
        var diffBuff = Buffer.from(diff1.div(new BN(this.difficulty)).toArray('be'));
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        ...
        case 'submit': {
            ...
            var job = miner.validJobs.filter((job)=> job.id === params.job_id)[0];
        ...
        if (!miner.proxy) {
            ...
            var blockTemplate = currentBlockTemplate.height === job.height ? currentBlockTemplate : validBlockTemplates.filter((t)=> t.height === job.height)[0];
...
function newConnectedWorker(miner){
    ...
    redisClient.hincrby(config.coin + ':active_connections', miner.login + '~' + miner.workerName, 1, (error, connectedWorkers) => {
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    redisClient.multi(redisCommands).exec((err, replies)=> {
        if (err){
        ...
        if (blockCandidate){
            ...
            var totalScore = Object.keys(workerScores).reduce((p, c)=> p + parseFloat(workerScores[c]), 0);
            var totalShares = Object.keys(workerShares).reduce((p, c)=> p + parseInt(workerShares[c]), 0);
...
function processShare(miner, job, blockTemplate, params){
    ...
    var hashNum = new BN(Buffer.from(hashArray), 'be');
...
function startPoolServerTcp(callback){
    ...
    async.each(config.poolServer.ports, (portData, cback)=> {
        var handleMessage = (socket, jsonData, pushMessage)=> {
            if (!jsonData.id) {
        ...
        var socketResponder = (socket)=> {
            socket.setKeepAlive(true);
            socket.setEncoding('utf8');

            var dataBuffer = '';

            var pushMessage = (method, params)=> {
                if(!socket.writable) return;
                var sendData = JSON.stringify({
                    jsonrpc: "2.0",
                    method: method,
        ...
        if (portData.ssl) {
            if (!config.poolServer.sslCert) {
                ...
                tls.createServer(options, socketResponder).listen(portData.port, (error, result) => {
                    if (error) {
            ...
            net.createServer(socketResponder).listen(portData.port, (error, result) => {
                if (error) {
