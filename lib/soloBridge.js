...
// Adjust logging level based on SOLO_DEBUG flag before loading logger
if (global.SOLO_DEBUG) {
    ...
    if (config.logging) {
        ...
        if (config.logging.files) {
            config.logging.files.level = 'debug';
        }
    }
}

// Load log system (must be after config is loaded)
require('./logger.js');
...
function logMetrics() {
    ...
    // Log metrics with [METRIC] in blue
    // Build the message with color already applied to avoid color loss in formatting
    const metricLabel = clc.blue('[METRIC]');
    const message = `${metricLabel} Accepted: ${metrics.shares.accepted}, Rejected: ${metrics.shares.rejected}, Blocks: ${metrics.shares.blocks}, Uptime: ${hours}h ${minutes}m ${seconds}s, Miners: ${Object.keys(metrics.miners).length}, Acceptance: ${acceptanceRate}%${minerInfo}`;
    log('info', logSystem, message);
}
